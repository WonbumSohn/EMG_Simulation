...
def parse():
    ...
    args.add_argument('--fs', type=float, help='frequency of EMG signal', default=2048)
    ...
    args.add_argument('--hdsemg_channels', type=int, nargs='+', help='channel size of HDsEMG', default=[8, 8])
    # Save parameters
    args.add_argument('--rslt_path', type=str, help='path of result', default='/mnt/d/NM/03_Programing/Simulation/NeuroMotion/Results')
    args.add_argument('--save_subj_name', type=str, help='name of subject', default='S01')
    args.add_argument('--save_task_name', type=str, help='name of task', default='isometric')
    args.add_argument('--save_mvc_level', type=str, help='mvc percentage level', default='None')
...
if __name__ == '__main__':
    for _ in range(5):
        ...
        para_config = parse()
        signal = {'signal': {}}
        save_path = os.path.join(para_config.rslt_path, para_config.save_subj_name, para_config.must_ms_label, para_config.save_task_name,
                                para_config.save_mvc_level)
        ...
        plot_spike_trains(spikes, os.path.join(save_path, 'Spike_trains_{}_{}.png'.format(para_config.must_ms_label, run_start_time)))
        ...
        # Model (use the pretrained BioMime model)
        cfg = update_config(para_config.cfg)
        print(cfg.Model.Generator['Dec']['final_interpolate'][-len(para_config.hdsemg_channels):])
        cfg.Model.Generator['Dec']['final_interpolate'][-len(para_config.hdsemg_channels):] = para_config.hdsemg_channels
        print(cfg.Model.Generator['Dec']['final_interpolate'][-len(para_config.hdsemg_channels):])
        ...
        for sp in tqdm(range(steps), dynamic_ncols=True, desc='Simulating MUAPs during dynamic movement...'):
            ...
            if not para_config.morph:
                ...
                base_muaps = base_muaps.to(device)

            if para_config.morph:
                sim = generator.generate(base_muaps, cond.float().to(device))
            else:
                sim = generator.sample(num_mus, cond.float().to(device), device, zi) #   Generate MUAPs using the pretrained BioMime model
        ...
        # plot muaps
        plot_muaps(muaps, save_path, np.arange(0, num_mus, 20), np.arange(0, steps, 5), suffix=para_config.must_ms_label, time=run_start_time)
        ...
        for i, col in enumerate(col_indices):
            plt.plot(t, emg[row, col] + i * np.max(np.abs(emg)), color=colors[i], label=f'Ch {(col + 1)}')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(save_path, f'emg_multi_{para_config.must_ms_label}_{run_start_time}.png'))
        # plt.show()
        plt.close()

        # Save .mat file with signal    
        signal['signal']['fsamp'] = para_config.fs
        signal['signal']['ngrid'] = 1
        signal['signal']['emgtype'] = 1
        signal['signal']['grid_size'] = [n_row, n_col]
        # save .mat file
        savemat(os.path.join(save_path, f'HDsEMG_signal_{run_start_time}.mat'), signal)
